...
> = {
  ...
  info: { bg: "bg-blue-50", text: "text-blue-800", icon: "i" },
  warning: { bg: "bg-amber-50", text: "text-amber-800", icon: "!" },
  custom: { bg: "bg-white", text: "text-slate-800" },
};
...
export function ToastProvider({ children }: { children: React.ReactNode }) {
  ...
  const scheduleRemovalAfterAnim = useCallback((id: string) => {
    ...
    const removeTimer = window.setTimeout(() => {
      ...
      if (timersRef.current[id] && Object.keys(timersRef.current[id]).length === 0) {
        delete timersRef.current[id];
      }
    }, ANIM_MS);
    timersRef.current[id] = { ...(timersRef.current[id] ?? {}), remove: removeTimer };
  }, []);
  ...
  const showToast = useCallback(
    (text: string, opts: ToastOptions = {}) => {
      ...
      const enterTimer = window.setTimeout(() => {
        ...
        timersRef.current[id] = { ...(timersRef.current[id] ?? {}), auto: autoTimer };
      }, 20); // small delay to ensure transition triggers

      timersRef.current[id] = { ...(timersRef.current[id] ?? {}), enter: enterTimer };
...
function ToastView({ toast, onClose }: { toast: ToastItem; onClose: () => void }) {
  ...
  return (
    ...
    >
      <div
        className={`flex w-full items-start gap-3 rounded-md border border-gray-100 px-3 py-2 shadow-sm ${bgClass}`}
      >
        {/* Icon */}
        <div className={`flex h-8 w-8 shrink-0 items-center justify-center rounded-full ${variantStyle.bg}`}>
          <span className={`text-sm font-semibold ${textClass}`}>{variantStyle.icon}</span>
        </div>
        ...
        <div className="flex-1">
          ...
          {/* small meta */}
          <div className="mt-1 text-xs text-slate-400">Auto-hide in {Math.round(duration / 1000)}s</div>
        </div>
